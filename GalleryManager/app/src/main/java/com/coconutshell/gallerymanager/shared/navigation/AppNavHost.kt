...
fun AppNavHost(startDestination:String="home") {
    ...
    fun openViewer(uri:String){nav.navigate("viewer/${Uri.encode(uri)}")}
    NavHost(nav,startDestination){
        ...
        composable("albums"){AlbumsScreen(onHome={nav.navigate("home"){popUpTo("home")}},onBrowse={nav.navigate("browse")},onOpenAlbum={nav.navigate("album/$it")})}
        composable("browse"){BrowserScreen(
            onHome={nav.navigate("home"){popUpTo("home")}},
            onAlbums={nav.navigate("albums")},
            onOpen=::openViewer,
            onFolder={nav.navigate("folder/$it")}
        )}
        composable("folder/{id}",arguments=listOf(navArgument("id"){type=NavType.LongType})){b->FolderDetailScreen(b.arguments!!.getLong("id"),{nav.popBackStack()},::openViewer)}
        composable("album/{id}",arguments=listOf(navArgument("id"){type=NavType.LongType})){b->AlbumDetailScreen(b.arguments!!.getLong("id"),{nav.popBackStack()},::openViewer)}
        composable("viewer/{uri}",arguments=listOf(navArgument("uri"){type=NavType.StringType})){b->
            val uri=Uri.decode(b.arguments!!.getString("uri")!!)
            ViewerScreen(uri,{nav.popBackStack()},{nav.navigate("info/${Uri.encode(uri)}")})
        }
        composable("info/{uri}",arguments=listOf(navArgument("uri"){type=NavType.StringType})){b->FileInfoScreen(Uri.decode(b.arguments!!.getString("uri")!!),{nav.popBackStack()})}
        ...
        composable("editor/{uri}",arguments=listOf(navArgument("uri"){type=NavType.StringType})){b->ImageEditorScreen(Uri.decode(b.arguments!!.getString("uri")!!),{nav.popBackStack()})}
