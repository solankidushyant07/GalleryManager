...
interface FileDao {
    ...
    @Query("SELECT * FROM files WHERE name LIKE '%' || :query || '%' OR mimeType LIKE '%' || :query || '%' ORDER BY dateModifiedEpochSeconds DESC")
    suspend fun search(query: String): List<FileRecordEntity>
    @Query("DELETE FROM files WHERE id NOT IN (:ids)") suspend fun deleteMissing(ids: List<Long>)
    @Query("DELETE FROM files") suspend fun deleteAll()
    @Upsert suspend fun upsertAll(items: List<FileRecordEntity>)
